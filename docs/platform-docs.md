...
- The local `publish-entry` runtime now uses the service-only repository and durable orchestration kernel. Hosted activation remains blocked on schema reconciliation, generated-type validation and coordinated deployment; no hosted schema or provider account has been changed.
- The production build resolves to the shared Intel Hub Supabase project. Read-only inventory on 18 July 2026 found that project inactive and its deployed `publish-entry` still on unauthenticated legacy version 1; the separate standalone Content Hub project is also inactive but is not the runtime target.
