...
_Production checkpoint: 2026-07-23. All seven PM Hermes write canaries and the hardened report-evidence rollout are reconciled; re-run `/update-data-reference` after future schema or agent-contract changes._
...
- `monthly_reports` keeps its shared signed-in team model, but each CRUD policy now requires a non-null `auth.uid()`. The anonymous role has no table privileges, while `service_role` retains CRUD for the governed agent boundary.
...
- The reviewed `approve-entry` v36, `send-notification` v40 and `content-hub-agent` v36 bundles are active on the canonical production project. Signed reads, bounded reporting and protected saved-report evidence are live for PM Hermes. The HMAC client signs the function-local `/content-hub-agent` path verified after the Supabase gateway rewrite. Migration `20260723163337_preserve_pm_hermes_report_evidence` is recorded under the exact repository version.
...
- Migration `20260723174659_harden_monthly_reports_authenticated_rls` preserves shared report CRUD for signed-in team members while requiring an explicit authenticated identity, removes anonymous table privileges and retains governed `service_role` access. The production Supabase adviser now reports zero findings for `monthly_reports`; the protected trigger still preserves `agent_evidence` across browser writes and only the service action path may replace it.
