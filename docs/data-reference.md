...
- The internal application still grants authenticated users broad CRUD access to `monthly_reports`, which Supabase's security adviser reports as permissive RLS. This is the existing team-access model, not added agent authority: the protected trigger preserves `agent_evidence` across browser writes and only the service action path may replace it.
