...
- Proposal and execution flags remain independent at both PM Hermes and Edge layers. The exact-approved `create_entry` canary produced one Pending/Draft entry with PM Hermes provenance and no approval or publication state; the separately approved `update_entry` canary advanced that Draft to revision 2 without changing its workflow authority. Execution returned to disabled after each action. Proposal-only access now exposes just an inert `create_idea` canary which has changed no Ideas record and still requires Dan's exact one-time `execute <action-id>` confirmation. Other action types remain disabled until their attended canaries pass.
