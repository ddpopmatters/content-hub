...
- Proposal and execution flags remain independent at both PM Hermes and Edge layers. The exact-approved `create_entry` canary produced one Pending/Draft entry with PM Hermes provenance and no approval or publication state, after which execution returned to disabled. Proposal-only access has advanced to a revision-bound `update_entry` canary; it changed no application record and still requires Dan's exact one-time `execute <action-id>` confirmation. Other action types remain disabled until their attended canaries pass.
