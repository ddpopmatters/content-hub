...
- Proposal and execution flags remain independent at both PM Hermes and Edge layers. The exact-approved `create_entry` canary produced one Pending/Draft entry with PM Hermes provenance and no approval or publication state; the separately approved `update_entry` canary advanced that Draft to revision 2 without changing its workflow authority. The exact-approved `create_idea` canary then produced one internal Ideas record with PM Hermes provenance. Execution returned to disabled after every action. Proposal-only access now exposes just a timestamp-bound `add_comment` canary which has not changed the Draft and still requires Dan's exact one-time `execute <action-id>` confirmation. Review-submission and reporting action types remain disabled until their attended canaries pass.
